...
- 管理员可调整人工占比、AI 占比和历史记录占比，控制三类来源对赔率的影响。
- 管理员记录实际着装后，竞猜锁定并可结算。
- 管理员可删除某天实际着装记录，系统会删除当天结算并恢复该日期为可下注状态。
...
单个选项的初始概率来自三类来源：人工维护概率、AI 预测概率、历史记录概率。管理员可在管理页调整三者比例，例如人工 40%、AI 40%、历史 20%。

```text
混合概率 = 人工概率 * 人工占比 + AI概率 * AI占比 + 历史概率 * 历史占比
```

AI 请求会包含人工占比、目标日前一天的实际着装、管理页配置的天气地区、管理页填写的天气预报/额外信息、以及历史着装记录。提示词会要求 AI 把人工占比当成衣柜/常见情况先验，并在能力允许时根据地区自行查询或推断目标日期天气。

历史概率使用时间衰减贝叶斯更新。越近的记录权重越高，越久远权重越低；实际着装中记录为“未知”的字段不会参与概率训练。历史占比设为 20% 时，历史记录最多只通过这 20% 来源影响单项概率。
...
删除历史记录时，系统会同时删除该日期的结算流水，并把该日期加入“开放下注”列表。这样过去或未来测试日期都可以恢复为可下注、可修改竞猜的状态；再次保存该日期实际着装后会重新锁定。
